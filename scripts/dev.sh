...
cleanup() { trap - EXIT INT TERM; for pid in "${api_pid:-}" "${web_pid:-}"; do [[ -n "$pid" ]] && kill "$pid" 2>/dev/null || true; done; npx astro dev stop >/dev/null 2>&1 || true; wait "${api_pid:-}" "${web_pid:-}" 2>/dev/null || true; }
...
INTERNAL_API_URL=http://127.0.0.1:8787 npm run dev -- --host 127.0.0.1
npx astro dev logs --follow & web_pid=$!
