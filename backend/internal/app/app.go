...
func New(cfg config.Config, db *sql.DB, logger *slog.Logger) http.Handler {
	...
	turnstile := auth.Turnstile(auth.SiteVerifier{Secret: cfg.TurnstileSecret, Client: &http.Client{Timeout: 5e9}})
	if cfg.Environment != "production" {
		turnstile = auth.DevelopmentVerifier{}
	}
	authService := &auth.Service{DB: db, SessionPepper: cfg.SessionPepper, OTPPepper: cfg.OTPPepper, Mailer: mailer, Turnstile: turnstile, SecureCookies: cfg.Environment == "production"}
	handler := (&api.API{DB: db, Auth: authService, Editorial: &editorial.Service{DB: db}, Logger: logger, Origin: cfg.PublicOrigin, AllowedOrigins: cfg.AllowedOrigins, MediaDir: cfg.MediaDir, MailMode: cfg.MailMode, Development: cfg.Environment != "production"}).Routes()
